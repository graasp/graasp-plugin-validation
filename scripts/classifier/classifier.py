"""
This script is modified based on the replication provided by Davidson et al. in their repo: https://github.com/t-davidson/hate-speech-and-offensive-language 
Paper of original author: https://aaai.org/ocs/index.php/ICWSM/ICWSM17/paper/view/15665
...
def other_features(tweet):
    ...
    twitter_objs = count_twitter_objs(tweet) #Count URL
...
def transform_inputs(tweets, tf_vectorizer, idf_vector):
    """
    ...
    M = np.concatenate([tfidf_array, oth_array],axis=1)
...
def get_tweets_predictions(tweets, perform_prints=True):
    ...
    model = load('pretrained_model.joblib')

    print ("Loading other information...")
    tf_vectorizer = load('pretrained_tfidf.pkl')
    idf_vector = load('pretrained_idf.pkl')

    print ("Transforming inputs...")
    X = transform_inputs(tweets, tf_vectorizer, idf_vector)
...
if __name__ == '__main__':
    ...
    data = df['Text']
    print(data[:5])
    true_class = df['class'].to_numpy()
    print(true_class[:5])
    data = [x for x in data if type(x) == str]
    data_predictions = get_tweets_predictions(data)
    ...
    for i,t in enumerate(data):
        print(t)
        print(class_to_name(data_predictions[i]))
