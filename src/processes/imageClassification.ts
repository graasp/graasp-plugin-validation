...
import fs from 'fs';
import { ItemValidationStatuses, PREDICTION_THRESHOLD } from '../constants';
import { FaildImageClassificationRequestError } from '../errors';
import { classifierEndpoint } from '../plugin';
...
export const sendRequestToClassifier = async (encodedImage: string) => {
  ...
  const response = await axios.post(classifierEndpoint, {
    data,
  })
  .then((response) => {
